...
void BilinearWorker::start()
{
    ...
    if (getSourceImage2() == nullptr || getSourceImage1()->getWidth() > getSourceImage1()->getHeight()) {
        ...
        if (getW() == 0 && getH() == 0)
            ...
            smallPixmap = QPixmap::fromImage(getSourceImage1()->getQImage()).scaledToHeight(getH(), Qt::SmoothTransformation);
        else if (getH() == 0)
            smallPixmap = QPixmap::fromImage(getSourceImage1()->getQImage()).scaledToWidth(getW(), Qt::SmoothTransformation);
        else
            smallPixmap= QPixmap::fromImage(getSourceImage1()->getQImage()).scaled(getW(),getH(),Qt::KeepAspectRatio, Qt::SmoothTransformation);
    }
    else {
        ...
        if (getW() == 0 && getH() == 0)
            ...
            smallPixmap = (pixmap->scaledToHeight(getH(), Qt::SmoothTransformation));
        else if (getH() == 0)
            smallPixmap = (pixmap->scaledToWidth(getW(), Qt::SmoothTransformation));
        else
            smallPixmap= (pixmap->scaled(getW(),getH(),Qt::KeepAspectRatio, Qt::SmoothTransformation));
    }
    emit pixmapReady(qimage, smallPixmap, getI());
    emit finished();
}

void BilinearWorker::startZoom()
{
    qInfo() << "startZoom";
    emit zoomReady(QPixmap::fromImage(getSourceImage1()->getQImage()).scaled(getW(), getH(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
    emit finished();
}
